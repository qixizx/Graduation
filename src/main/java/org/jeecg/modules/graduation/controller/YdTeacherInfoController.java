...
public class YdTeacherInfoController {
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      ...
      List<YdTeacherInfo> pageList = ydTeacherInfoService.list(queryWrapper);
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "老师信息表列表");
      mv.addObject(NormalExcelConstants.CLASS, YdTeacherInfo.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("老师信息表列表数据", "导出人:Jeecg", "导出信息"));
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              ...
              ydTeacherInfoService.saveBatch(listYdTeacherInfos);
              return Result.ok("文件导入成功！数据行数:" + listYdTeacherInfos.size());
          } catch (Exception e) {
              log.error(e.getMessage(),e);
